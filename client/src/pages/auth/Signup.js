...
const SignupSchema = yup.object().shape({
    ...
    password: yup.string().min(5, "Password min of 5 characters").required("Password is required") ,//include good reg ex matching.
    image: yup.string(),
});
        ...
        return Object.keys(values).map((formValue)=> {
            return (
                <div 
                    ...
                    className="field"
                >
                    <label className='auth-label' htmlFor={formValue}>{valueMap[formValue].title} {formValue !== 'image'&&<span className="required">*</span>}</label>
                    <i class={"auth-field-icon "+valueMap[formValue].icon}></i>
                    <input 
